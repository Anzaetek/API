...
q0 = SolverProblem(tg.Target([], [], {}, {}, {}), 
                   ...
                   ['x1', 'x2'], 
                   {'l': 1.0, 'v': 100.0, 'v2': 1000.0},
                   config_m02_03bis, UserTokenSerde(user1, token1)) # type: ignore
q1 = makeSolverProblemSerde(q0)
print(q1)
print(JSONSerializer.serialize(q1))
q2 = makeSolverProblem(q1)
...
def queryDone2(x: Any) -> None:
    print(x)
    print(x.Results)
    print(x.Results['p'])
    print(x.Results['x1'])
    print(x.Results['x2'])            
    if np.allclose(x.Results['p'][0:2], np.array([1.0, 0.2]), 5e-3):
        print("ok")
    else:
        raise Exception('failure')
    if np.allclose(x.Results['x1'], np.array([0.2]), 5e-3):
        print("ok")
    else:
        raise Exception('failure x1')
    if np.allclose(x.Results['x2'], np.array([1.0]), 5e-3):
        print("ok")
    else:
        raise Exception('failure x2')  

pquerySolverProblem("https://api2.anzaetek.com:443/execute", q0, queryDone2) 