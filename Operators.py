...
_TermQuadratic_ : Callable[[Any,Any],"Term"] = lambda x,y: Term()
...
class TermQuadratic(Term):
    ...
    def __init__(self, m:Any, x:Any) -> None:
        self.tag = self.getTag()
...
class OptTerm(Term):
    def getTag(self)->int:
        return TERM_OPT    
    def __init__(self, quad:Any, lin:Any, op: int, cmp: Any, label: str) -> None:
        self.quad = quad
        self.lin = lin
        self.op = op
        self.cmp = cmp
...
class Target(Term):
    def getTag(self)->int:
        return TERM_TARGET    
    def __init__(self, components: List[OptTerm], annotations: List[Term], positions: Dict[str,int], sizes: Dict[str,int]) -> None:
...
def maximize(v: Any, expr: Any)->Term:
    return TermMaximize(v,expr)

def minimize(v: Any, expr: Any)->Term:
    return TermMinimize(v,expr)

def sum(expr:Any)->Term:
    return cast("Term", wrap(1)*expr)

def minimum(expr:Any)->Term:
    return TermMinimum(expr)

def maximum(expr:Any)->Term:
    return TermMaximum(expr)

def declare(a:Any,b:Any)->Term:
    return TermDeclare(a,b)

def label(a:Any,b:Any)->Term:
    return TermLabel(a,b)

def bitsize(a:Any,b:Any)->Term:
...
def propagateShape(h: PositionsHints, t: Term, sh: Tuple[int,int]) -> None:
    tag = t.getTag()
    if tag == TERM_ATOM:
        ta = cast(TermAtom, t)
        a = ta.atom
        if isinstance(a, np.ndarray):
            ...
            if len(ss) == 2:
                if ss == (1,1):
                    v = cast(float, a[0,0])
                    a = np.diag([v for k in range(sh[0])], dtype=np.float64) # type: ignore
                    ta.atom = a
        ...
        if isinstance(a, int) or isinstance(a, float) or isinstance(a, np.float32) or isinstance(a, np.float64):
            if sh[1]==1:
                ...
                a = np.diag([a for k in range(sh[0])], dtype=np.float64) # type: ignore
                ta.atom = a
...
def fillSlots(sizes: Dict[str,int], positions: Dict[str, int])->List[str]:
    ...
    #print("sorted", szs)
    def findPosition(n: str, s:int) -> int:
        if n in positions:
            return positions[n]
        cursor = 0
        failedAt = 0
...
def findSizeAndPositions(h: PositionsHints, t: Term) -> None:
    #print(t)
...
def symmetrize(m: np.ndarray)->np.ndarray:
    return cast(np.ndarray, 0.5*(m+np.transpose(m)))
...
def extractMultiDimLinearForm(h: PositionsHints, t: Term, windowBase: int, windowSize:int)->Tuple[np.ndarray, np.ndarray]:
    ...
    if tag == TERM_MUL:
        ...
        def sizeUp(mm:np.ndarray, Ax:Any) -> np.ndarray:
            #print("sizeUp", mm, Ax)
            # find indices, copy
        ...
        if mm1 is None:
            if mm2 is None:
                ...
                # will it even work?
                return (np.matmul(np.transpose(cast(np.ndarray,By)),cast(np.ndarray,Ax)) 
                        + np.matmul(np.transpose(cast(np.ndarray,Bx)),cast(np.ndarray,Ay)), np.matmul(np.transpose(cast(np.ndarray,Bx)), cast(np.ndarray,By)))
            else:
                # (Ax,Bx) (x) matrix ~ matrix^t * (Ax,Bx)
                mm2_ = sizeUp(mm2, Ax)
                mm2t = np.transpose(mm2_)
                ##print("mul/757", mm2t, Ax, Bx)
                return (np.matmul(mm2t,cast(np.ndarray,Ax)), np.matmul(mm2t, cast(np.ndarray,Bx)))
        else:
            if mm2 is None:
                mm1_ = sizeUp(mm1, Ay)
                ##print("mul/785", mm1_, Ay, By)
                return (np.matmul(mm1_, cast(np.ndarray,Ay)), np.matmul(mm1_,cast(np.ndarray,By)))
            else:
                # panic!
...
#                & op.label('v', x2 == 1.0)
#                & op.label('v2', x1 > 0.2))
#        target = top.expressionToTarget(expr, {'l': 1.0, 'v': 100.0, 'v2': 1000.0})
#        print(target)     
#        e = ExtractQUBO(target, config_m02, UserTokenSerde(config_m01_ut["user"], config_m01_ut["token"]))   
#        q = extractQUBO(e)
#        print(q)

def tolist(*l: Any) -> List[Any]:
    return [*l]

def at(x:Any,y:Any)->Any:
    return x[y]

def evalExpr(m: str, nameslist: List[str], vars: Dict[str, Any])->Any:
    names = vars.copy()
    for l in nameslist:
        names[l] = sym(l)
    evaluator = SimpleEval(functions = {
        "sym": sym,
        "wrap": wrap,
        "quadratic": quadratic,
        "maximize": maximize,
        ...
        "TermLt": TermLt,
        "TermAnd": TermAnd,
        "TermGetItem": TermGetItem,
        "TermDeclare": TermDeclare,
        "TermBitsize": TermBitsize,
        "TermLabel": TermLabel,
        "TermQuadratic": TermQuadratic,
        "TermMaximize": TermMaximize,
        "TermMinimize": TermMinimize,
        "TermMaximum": TermMaximum,
        "TermMinimum": TermMinimum,
        "array": np.array       
    }, names=names)
    evaluator.operators[ast.BitAnd] = operator.and_      
    evaluator.operators[ast.Pow] = operator.pow 
    evaluator.operators[ast.Mult] = operator.mul
    evaluator.operators[ast.Add] = operator.add               
    expr = evaluator.eval(m)
    return expr

if __name__ == '__main__':
    expr = "label('l', minimize(list(x1,x2), (x1 - 0.1)**2)) & label('v', x2 == 1.0) & label('v2', x1 > 0.2)"
    e2 = evalExpr(expr, ['x1', 'x2'], {'l': 1.0, 'v': 100.0, 'v2': 1000.0})
    print(e2)